...
inline evmc_status_code balance(ExecutionState& state) noexcept
{
    ...
    if (state.rev >= EVMC_BERLIN && state.host.access_account(addr) == EVMC_COLD_ACCESS)
    {
...
inline evmc_status_code selfdestruct(ExecutionState& state) noexcept
{
    ...
    if (state.rev >= EVMC_BERLIN && state.host.access_account(beneficiary) == EVMC_COLD_ACCESS)
    {
